/**
 * Solar_system (planet model) module for MagicMirror.
 */
Module.register("solar_system", {
	...
	defaults: {
		...
		earthColor: 'skyblue',
		marsColor: 'orangered',
		jupiterColor: 'yellowgreen',
		saturnColor: 'greenyellow',
		uranusColor: 'blue',
	...
	getDom: function () {
		...
		canvas.setAttribute("id", "solar_system" + date.getTime());
		...
		var maxX = 0;
		var maxY = 0;
		for (var i = 0, a = this.planets; i < a.length; i++) {
		    var planet = a[i];
		...
		if (this.config.showOrbits) {
			...
			var neptune = this.planets[this.planets.length - 1];
			for (var i = 0; i < neptune.orbitNumSteps; ++i, date.setDate(date.getDate() + neptune.orbitStepDays)) {
				var xyz = getXyzForPlanet(neptune.name, date);
		...
		if (this.config.showOrbits) {
		for (var j = 0, a = this.planets; j < a.length; j++) {
		    var planet = a[j];
		    var date = new Date();
		    ctx.strokeStyle = this.config.orbitColor;
		    ctx.beginPath();
		    for (var i = 0; i < planet.orbitNumSteps; ++i, date.setDate(date.getDate() + planet.orbitStepDays)) {
		        var xyz = getXyzForPlanet(planet.name, date);
		        xyz.scale(this.getPlanetScale(planet.name, 1.0));
